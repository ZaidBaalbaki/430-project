...
def Clinic_edit(request,id):
    ...
    if request.method == 'POST':
        ...
        if not filter_obj:
            ...
            messages.success(request,'success')
            return redirect(f'/Clinic/{id}/')
    return render(request,'Clinic_edit.html',{'profile':profile,'title':'Edit Profile'})
...
def count(request,hos_id):
    ...
    available_doctor_count=Doctor_profile.objects.filter(Clinic_id=hos_id,is_available=True).count()   
